...
class JugeCRUDController extends Controller
{
  public function get(Request $request){
    ...
    //Get data
    $data = method_exists($model, 'jugeGet') ? $model->jugeGet($request) : $model->get();
  ...
  public function getKeys(Request $request){
    ...
    if(count($modelKeys) == 0){
      $data = method_exists($model, 'jugeGet') ? $model->jugeGet([]) : $model->get();
  ...
  public function getPostInputs(Request $request){
    ...
    if(method_exists ( $model , 'jugeGetPostInputs' )){
      ...
      //Remove required on password
      foreach ($modelInputs as $k => $input) {
        if($input['name'] == 'password'){
          $modelInputs[$k]['required'] = false;
        }
      }

...
<div>
  ...
  <div class="pre-table-wrapper my-2" style=""> 
    ...
    <div class="pre-table-box box-search" style="">
      <div class="d-flex">
        <juge-search-filter v-if="0" class="mx-2" :model="cDataModelSingle" />
        <juge-id-filter v-if="0" class="mr-2" :model="cDataModelSingle" />
      </div>
    </div>  
    <!-- Paginator -->
    ...
    <div class="pre-table-box box-settings" style="">
      <button @click="stacked=!stacked" class="btn btn-info btn-sm d-lg-none" style="margin-right: 10px; height: 38px;">📄</button>
  ...
  >
   <!-- @sort-changed="sort" -->
    ...
    <template v-slot:cell()="data">
      ...
      <!-- Color -->
      <span v-else-if="data.field.type == 'color'">
      ...
      <span v-else-if="data.field.type == 'image'">
        <img :src="data.value" alt=""  style="height: 75px;">
      </span>
      <!-- Images -->
  ...
  {{/*No records*/}}
  <div v-if="cKeys && cData.length == 0" class="d-flex mb-2" :style="'justify-content: space-around;'" ><span>No records 🤷‍♂️</span></div>
